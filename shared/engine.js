...
function emit(g, type, details = {}) { g.events.push({ id:g.nextEvent++, type, ...details }); }
...
export function createGame(faction='vampire',random=Math.random,mode='practice',deckLists={}) {
  check(Object.hasOwn(DECKS,faction),'Facção inválida.');
  const factions=[faction,faction==='vampire'?'werewolf':'vampire'];
  const g={round:1,turn:0,initiative:0,phase:'playing',winner:null,nextId:1,nextEvent:1,version:0,mode,riskMode:'covenant',events:[],log:['A caçada começou.'],players:factions.map(f=>({faction:f,health:24,maxHealth:24,heroGear:[],heroAttackBonus:0,renown:0,energy:3,rage:0,combo:0,skillUsed:false,ultimateUsed:false,kills:0,cardsPlayed:0,claims:0,deck:shuffle(deckLists[f]||DECKS[f],random),hand:[],lanes:emptyLanes(),boosts:{court:0,crypt:0,hunt:0},passed:false,fatigue:0}))};
...
function heal(g,seat,amount) {const p=g.players[seat],actual=Math.min(amount,(p.maxHealth||24)-p.health);if(actual>0){p.health+=actual;emit(g,'heal',{seat,target:'hero',amount:actual});}}
...
function strike(g,seat,action) {
  ...
  if(CARDS[u.cardId].keyword==='lifesteal'||gearHas(u.gearItems,'lifesteal'))heal(g,seat,dealt);
...
export function applyAction(original,actor,action) {
  ...
  if(action.type==='pass'){p.passed=true;g.log.push(`Combatente ${actor+1} encerrou a rodada.`);}
    ...
    if(action.type==='attack')strike(g,actor,action);
      ...
      if(gearHas(p.heroGear,'lifesteal'))heal(g,actor,dealt);
      ...
      if(p.faction==='vampire')heal(g,actor,ult?3:2);
      ...
      if(c.type==='unit'){check(units.length<3,'Esta frente já possui três unidades.');const u=unit(g,c.id);units.push(u);emit(g,'summon',{seat:actor,target:u.uid,lane:action.lane,label:c.name});triggerSynergies(g,actor,action.lane,'ally-summoned',{faction:c.faction,exclude:u.uid});}
        ...
        if(c.effect==='drain')heal(g,actor,dealt);
  ...
  g.version++;g.log=g.log.slice(-60);g.events=g.events.slice(-180);return g;
}
export function botAction(g) {
  const actor=g.turn,p=g.players[actor],enemy=g.players[1-actor],options=[];
  function consider(action,score){try{const next=applyAction(g,actor,action);if(next.phase==='finished'&&next.winner===actor)score+=1000;options.push({action,score});}catch(e){if(!(e instanceof RuleError))throw e;}}
  for(const l of LANES){
    const foes=enemy.lanes[l.id];
    for(const u of p.lanes[l.id])if(u.ready!==false){
      if(!foes.length)consider({type:'attack',uid:u.uid,lane:l.id,target:'hero'},7+u.attack);
      else for(const target of foes)consider({type:'attack',uid:u.uid,lane:l.id,target:target.uid},3+Math.min(u.attack,target.health)+(u.attack>=target.health?4:0)-(target.attack>=u.health?3:0));
    }
    for(const type of ['skill','ultimate'])for(const target of ['hero',...foes.map(u=>u.uid)])consider({type,lane:l.id,target},type==='ultimate'?15:target==='hero'?2:5);
    for(const instance of p.hand){
      const c=CARDS[instance.cardId];let score=3;
      if(c.type==='unit')score=4+c.attack+c.health/2+(power(p,l.id)<=power(enemy,l.id)?2:0)+(c.synergy?1:0);
      if(c.type==='equipment')score=4+c.attack;
      if(c.effect==='heal')score=Math.min(3,p.maxHealth-p.health);
      if(c.effect==='damage'||c.effect==='execute'||c.effect==='rend')score=5;
      if(c.effect==='sacrifice')score=p.health>8&&p.hand.length<5?3:-1;
      if(c.effect==='pounce')score=6;
      if(EVENTS[g.round-1].lane===l.id)score+=0.5;
      consider({type:'play',uid:instance.uid,lane:l.id},score);
    }
  }
  options.sort((a,b)=>b.score-a.score);return options[0]?.score>0?options[0].action:{type:'pass'};
}
