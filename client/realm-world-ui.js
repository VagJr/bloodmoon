...
function objectArt(a){const id=BUILD_ART[a.blueprintId]||({wayshrine:'stone-well',satchel:'lost-satchel',caravan:'moon-caravan',portal:'dungeon-gate'})[a.kind];return id?'/assets/world/objects/'+id+'.png':a.kind==='resource'?RESOURCE_ART[a.resource]:null;}
...
function focusPlayer(smooth=false){const {w,h}=dimensions(),p=controller?.pose||dataRef.liveWorld.player;camera.x=w/2-p.x*60*camera.z;camera.y=h/2-p.y*SIZE.height/100*camera.z;clampCamera();paintCamera(smooth);}
function paintCamera(smooth=false){const ground=rootRef?.querySelector('.rw-map-backdrop');if(ground&&dataRef){const maps=dataRef.liveWorld.continents||[],{w}=dimensions(),center=(-camera.x+w/2)/(60*camera.z),continent=maps.findLast(m=>center>=m.offset)||maps[0];ground.style.backgroundImage=`url('${continent?.image||'/assets/world/vespera-map.png'}')`;ground.style.backgroundSize=`${6000*camera.z}px ${4000*camera.z}px`;ground.style.backgroundPosition=`${camera.x+(continent?.offset||0)*60*camera.z}px ${camera.y}px`;}const plane=rootRef?.querySelector('.rw-plane');if(plane){plane.style.transition='none';plane.style.transform=`translate(${camera.x}px,${camera.y}px) scale(${camera.z})`;}}
...
function visiblePoint(p,margin=250){const {w,h}=dimensions(),x=p.x*60*camera.z+camera.x,y=p.y*40*camera.z+camera.y;return x>-margin&&x<w+margin&&y>-margin&&y<h+margin;}
...
function paintWorld(){
 ...
 const people=w.players.filter(p=>p.id!==w.player.publicId).map(p=>({...p,kind:'traveler'})),entities=[...w.actors,...people].filter(a=>visiblePoint(a,140));
...
async function send(input){const session=controller,callbacks=handlers;if(!session||session.commandBusy)return;session.commandBusy=true;try{const d=await callbacks.sendAction(input);if(controller!==session)return;if(d){dataRef=normalizeData(d);if(d.liveWorld){if(['world-recover','world-ability'].includes(input.type)&&d.liveWorld.player.displacement!==session.displacement){session.displacement=d.liveWorld.player.displacement;session.pose={x:d.liveWorld.player.x,y:d.liveWorld.player.y};session.accumulatedMs=0;session.queuedMove=null;if(input.type==='world-recover')focusPlayer();}paint();}}const message=d?._worldResult?.message||d?.liveWorld?.lastResult?.message;if(message)feedback(message);return d;}catch(e){feedback(e.message||'A ordem não foi aceita.');return null;}finally{session.commandBusy=false;}}
...
function loop(now){
 ...
 if((d.dx||d.dy)&&!blocked){
  ...
  const self=rootRef.querySelector('.rw-self');if(self){self.style.left=session.pose.x*60+'px';self.style.top=session.pose.y*40+'px';self.classList.add('rw-moving');}
  const follow=1-Math.exp(-dt/65),size=dimensions();camera.x+=(size.w*.5-session.pose.x*60*camera.z-camera.x)*follow;camera.y+=(size.h*.48-session.pose.y*40*camera.z-camera.y)*follow;clampCamera();paintCamera();rootRef.style.setProperty('--facing',Math.atan2(d.dy,d.dx)+'rad');
 }else{
...
export function mountRealmWorld(root,data,callbacks){const surface=root.querySelector('.rw-world');if(!surface)return;if(rootRef!==surface){unmountRealmWorld();rootRef=surface;handlers=callbacks;controller={lastSentAt:0,accumulatedMs:0,inFlight:false,queuedMove:null,lastDir:{dx:0,dy:0},pose:{x:data.liveWorld.player.x,y:data.liveWorld.player.y}};dataRef=normalizeData(data);camera.z=innerWidth<=720?.6:.9;bind();paint();focusPlayer();mountActionHud(surface,{send,direction,pose:()=>controller.pose,regions:()=>dataRef.regions,blocked:()=>isBlocked(),target:()=>selectedTarget,select:id=>{selectedTarget=id;slotTarget=null;},avatar:()=>dataRef.player.avatar||'vesper',name:()=>dataRef.profile.name,stopMovement:clearKeys,reconcile:()=>{controller.pose={x:dataRef.liveWorld.player.x,y:dataRef.liveWorld.player.y};focusPlayer();},viewport:()=>rootRef.querySelector('.rw-viewport').getBoundingClientRect(),zoom:()=>camera.z,project:p=>({x:p.x*60*camera.z+camera.x,y:p.y*40*camera.z+camera.y}),unproject:(x,y)=>{const rect=rootRef.getBoundingClientRect();return {x:(x-rect.left-camera.x)/(60*camera.z),y:(y-rect.top-camera.y)/(40*camera.z)};},locate:n=>{camera.x=dimensions().w/2-n.x*60*camera.z;camera.y=dimensions().h/2-n.y*40*camera.z;paint();},interact:()=>{const w=dataRef.liveWorld,a=w.actors.find(a=>a.id===selectedTarget)||nearest(w);if(a){if(a.arenaKind)handlers.openEncounter(a.id);else send({type:'world-interact',targetId:slotTarget||a.id});}}},data.liveWorld);frame=requestAnimationFrame(loop);}else{handlers=callbacks;updateRealmWorld(surface,data);}}
