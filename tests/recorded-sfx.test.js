...
test('recorded cues reference all 27 unique imported samples',()=>{
 const imports=JSON.parse(readFileSync(new URL('../docs/audio/imported-sfx.json',import.meta.url)));
 const ids=new Set(Object.values(RECORDED_CUES).flatMap(c=>c.ids));assert.equal(ids.size,27);
 ...
 assert.equal(new Set(imports.files.map(a=>a.sha256)).size,27);
});
test('sample mixer preloads once, varies takes, bounds voices and stops on mute',async()=>{
 ...
 try{
  const mixer=createRecordedSfx(context,node());await mixer.warm();await mixer.warm();assert.equal(fetches,27);
