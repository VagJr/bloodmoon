...
test('campaign is playable through eight chapters and rewards cannot be claimed twice',()=>{
 ...
 for(const chapter of CHAPTERS.slice(1))action('chapter',{choice:chapter.choices[0][0]});
...
test('account persistence, legacy linking, logout, matchmaking and video streaming work end to end',async()=>{
 ...
 try{
  ...
  const onboard=await req('/api/onboarding','POST',{faction:'vampire'},b.cookie);assert.equal(onboard.status,200);assert.equal(onboard.data.profile.decks.length,2);assert.equal(onboard.data.profile.coins,300);
  const again=await req('/api/onboarding','POST',{faction:'werewolf'},b.cookie);assert.equal(again.data.profile.coins,300);assert.equal(again.data.profile.starterFaction,'vampire');
  ...
  const paired=await req('/api/matchmaking','POST',{faction:'vampire'},b.cookie);assert.equal(paired.data.state,'matched');
