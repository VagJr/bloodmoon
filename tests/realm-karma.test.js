...
test('caça progride as quatro insígnias sem alterar a linhagem',()=>{
 ...
 assert.equal(karmaView(hunter,now).rank,'red');
 for(let i=0;i<4;i++)recordCreatureKill(hunter,{kind:'raid',level:12},now+i+10);
 assert.equal(karmaView(hunter,now).rank,'black');
 for(let i=0;i<6;i++)recordCreatureKill(hunter,{kind:'raid',level:12},now+i+20);
 assert.equal(karmaView(hunter,now).rank,'gold');
 assert.equal(karmaView(hunter,now).bossKills,10);
