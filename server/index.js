...
const worldTimer=setInterval(()=>{
  ...
  exclusiveWorldTask(()=>{
    ...
    realmLivePulse(id=>{
      ...
      return profile?.realm?liveWorldFor(profile,now):null;
    },id=>{
...
server.listen(port, host, () => {
  ...
  console.log(
    `Bloodmoon HTTP ONLINE em http://${address.address}:${address.port} · ${mongoStore ? 'MongoDB Atlas' : 'persistência local'}`
  );
});
